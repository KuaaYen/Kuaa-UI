...
const GlitchEffectDemo = ({
    ...
    const cycleStartTime = useRef(0); // 記錄週期開始時間
    const updateInterval = fuzzInterval;
    
    // 週期設定
    const animationDuration = 4000; // 4秒動畫
    const restDuration = 600;      // 1秒休息
    const totalCycleDuration = animationDuration + restDuration; // 5秒總週期
    ...
    useEffect(() => {
        if (!shouldAnimate) {
            ...
            cycleStartTime.current = 0; // 重置週期時間
        }
